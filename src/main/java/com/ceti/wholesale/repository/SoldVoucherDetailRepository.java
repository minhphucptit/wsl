...
@Repository
public class SoldVoucherDetailRepository {
    ...
    public ResultPage<Object[]> findAllWithFilter(Pageable pageable, MultiValueMap<String, String> where) {
        ...
        String selectQuery = "select distinct {sold_voucher.*},{company.*},{customer.*}, {truck_driver.*}, {truck.*}, {salesman"
                + ".*} ";
        String fromQuery = "from sold_voucher left join company on sold_voucher.company_id = company.id left join "
                + "customer on sold_voucher.customer_id = customer.id left join truck_driver on sold_voucher"
                + ".truck_driver_id = truck_driver.id left join truck on sold_voucher.truck_license_plate_number = truck"
                + ".license_plate_number left join salesman on sold_voucher.salesman_id = salesman.id"
                + " left join goods_in_out on goods_in_out.voucher_id = sold_voucher.id";
        ...
        List<String> equalType = new ArrayList<>(Arrays.asList("company_id", "customer_id", "truck_driver_id", "truck_license_plate_number", "factory_id", "goods_in_out.type"));
        ...
        allowEntities.put("sold_voucher", SoldVoucher.class);
        ...
        return SqlProcessV2.getResultPageColumnWithEmbed(selectQuery, fromQuery, session, "sold_voucher", allowEntities, pageable,
